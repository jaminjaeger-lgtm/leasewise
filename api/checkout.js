export default async function handler(req, res) {
  ...
  try {
    const params = isSubscription ? new URLSearchParams({
      'payment_method_types[]': 'card',
      'line_items[0][price_data][currency]': 'usd',
      'line_items[0][price_data][product_data][name]': 'LeaseWise Unlimited',
      'line_items[0][price_data][product_data][description]': 'Unlimited document analyses, PDF downloads, priority support',
      'line_items[0][price_data][unit_amount]': '1500',
      'line_items[0][price_data][recurring][interval]': 'month',
      'line_items[0][quantity]': '1',
      'mode': 'subscription',
      'success_url': `${origin}/?unlocked=true&plan=subscription`,
      'cancel_url': `${origin}/`,
    }) : new URLSearchParams({
      'payment_method_types[]': 'card',
      'line_items[0][price_data][currency]': 'usd',
      'line_items[0][price_data][product_data][name]': 'LeaseWise Full Report',
      'line_items[0][price_data][product_data][description]': 'Complete document analysis with all flags, findings, and PDF download',
