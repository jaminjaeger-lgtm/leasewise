...
export default function LeaseWise() {
  ...
  const handleCheckout = async (planType = "single") => {
  ...
  const downloadPDF = () => {
    if (!results) return;
    const verdictLabel = verdictConfig[results.verdict].label;
    const flagLines = results.flags.map(f =>
      `[${f.type.toUpperCase()}] ${f.title}\n${f.detail}`
    ).join("\n\n");

    const text = `LEASEWISE REPORT
${"=".repeat(50)}
...
${"=".repeat(50)}
Generated by LeaseWise · leasewise.rentals
This report is for informational purposes only and does not constitute legal advice.`;
    ...
    a.href = url;
    a.download = "LeaseWise-Report.txt";
    a.click();
  ...
  const restart = () => {
    ...
    setError(""); setUnlocked(false); setIsSubscriber(false);
  };
          ...
          <>
                ...
                {!unlocked && lockedCount > 0 && (
                  <div style={{ position: "relative", marginTop: 4 }}>
                    ...
                    <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "1.25rem", textAlign: "center", marginTop: "0.5rem" }}>
                      ...
                      <div style={{ fontSize: "0.85rem", color: COLORS.inkMuted, marginBottom: "1.25rem", lineHeight: 1.5 }}>
                        Unlock the full report to see all findings, plus download a PDF copy to keep.
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: "0.75rem" }}>
                        <div style={{ border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "1rem", textAlign: "left" }}>
                          <div style={{ fontWeight: 600, fontSize: "0.95rem", color: COLORS.ink, marginBottom: 3 }}>One Report</div>
                          <div style={{ fontSize: "1.5rem", fontWeight: 700, color: COLORS.ink, marginBottom: 6 }}>$4</div>
                          <div style={{ fontSize: "0.75rem", color: COLORS.inkMuted, marginBottom: "0.875rem", lineHeight: 1.5 }}>Full report + PDF download for this document</div>
                          <button
                            onClick={() => handleCheckout("single")}
                            disabled={checkoutLoading}
                            style={{ background: COLORS.ink, color: COLORS.cream, border: "none", borderRadius: 6, padding: "0.6rem 0", fontFamily: "inherit", fontSize: "0.85rem", fontWeight: 500, cursor: checkoutLoading ? "not-allowed" : "pointer", width: "100%" }}
                          >
                            {checkoutLoading ? "Loading…" : "Unlock — $4"}
                          </button>
                        </div>
                        <div style={{ border: `2px solid ${COLORS.gold}`, borderRadius: 8, padding: "1rem", textAlign: "left", position: "relative" }}>
                          <div style={{ position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: COLORS.gold, color: "white", fontSize: "0.65rem", fontWeight: 600, padding: "2px 8px", borderRadius: 10, whiteSpace: "nowrap" }}>BEST VALUE</div>
                          <div style={{ fontWeight: 600, fontSize: "0.95rem", color: COLORS.ink, marginBottom: 3 }}>Unlimited</div>
                          <div style={{ fontSize: "1.5rem", fontWeight: 700, color: COLORS.ink, marginBottom: 6 }}>$15<span style={{ fontSize: "0.8rem", fontWeight: 400, color: COLORS.inkMuted }}>/mo</span></div>
                          <div style={{ fontSize: "0.75rem", color: COLORS.inkMuted, marginBottom: "0.875rem", lineHeight: 1.5 }}>Unlimited analyses + PDF downloads every month</div>
                          <button
                            onClick={() => handleCheckout("subscription")}
                            disabled={checkoutLoading}
                            style={{ background: COLORS.gold, color: "white", border: "none", borderRadius: 6, padding: "0.6rem 0", fontFamily: "inherit", fontSize: "0.85rem", fontWeight: 500, cursor: checkoutLoading ? "not-allowed" : "pointer", width: "100%" }}
                          >
                            {checkoutLoading ? "Loading…" : "Subscribe — $15/mo"}
                          </button>
                        </div>
                      </div>
                      <div style={{ fontSize: "0.72rem", color: COLORS.inkMuted }}>
            ...
            {unlocked && (
              <div style={{ display: "flex", gap: 10, marginTop: "1.25rem", flexWrap: "wrap" }}>
                <button
                  onClick={downloadPDF}
                  style={{ background: COLORS.ink, color: COLORS.cream, border: "none", borderRadius: 8, padding: "0.7rem 1.25rem", fontFamily: "inherit", fontSize: "0.875rem", fontWeight: 500, cursor: "pointer" }}
                >
                  Download Report
                </button>
                {isSubscriber && (
                  <button
                    onClick={restart}
                    style={{ background: COLORS.white, color: COLORS.ink, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "0.7rem 1.25rem", fontFamily: "inherit", fontSize: "0.875rem", fontWeight: 500, cursor: "pointer" }}
                  >
