...
export default function LeaseWise() {
  ...
  const downloadPDF = () => {
    ...
    script.onload = () => {
      ...
      const wrapText = (text, x, maxW, lineH) => {
      ...
      doc.text("LeaseWise", margin, 12);
      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.text("leasewise.rentals", pageW - margin, 12, { align: "right" });
      y = 30;

      // Context pill
      doc.setFontSize(8);
      doc.setTextColor(196, 146, 26);
      doc.text(`${state}  ·  ${docType === "template" ? "Blank Template" : "Completed Agreement"}`, margin, y);
      y += 10;
      ...
      const vc = vColors[results.verdict] || vColors.amber;
      doc.setFillColor(...vc);
      doc.circle(margin + 3, y - 2, 2, "F");
      doc.setTextColor(...vc);
      doc.setFontSize(8);
      doc.setFont("helvetica", "bold");
      doc.text(verdictConfig[results.verdict].label.toUpperCase(), margin + 8, y);
      y += 7;

      doc.setTextColor(26, 26, 24);
      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      const titleLines = doc.splitTextToSize(results.verdictTitle, contentW);
      titleLines.forEach(l => { doc.text(l, margin, y); y += 8; });
      y += 2;

      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(107, 107, 101);
      wrapText(results.verdictSummary, margin, contentW, 5.5);
      y += 8;
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(107, 107, 101);
      doc.text("PLAIN ENGLISH SUMMARY", margin, y);
      y += 6;
      doc.setFont("helvetica", "normal");
      doc.setTextColor(26, 26, 24);
      doc.setFontSize(9.5);
      wrapText(results.summary, margin, contentW, 5.5);
      y += 8;
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(107, 107, 101);
      doc.text("FLAGS & FINDINGS", margin, y);
      y += 7;

      const flagBg = { ok: [234, 245, 238], warn: [254, 243, 226], danger: [253, 236, 234] };
      const flagDot = { ok: [46, 125, 79], warn: [212, 112, 10], danger: [192, 57, 43] };

      results.flags.forEach(f => {
        const titleLines = doc.splitTextToSize(f.title, contentW - 12);
        const detailLines = doc.splitTextToSize(f.detail, contentW - 12);
        const blockH = 6 + titleLines.length * 5.5 + detailLines.length * 5 + 6;
        checkPage(blockH + 4);

        const bg = flagBg[f.type] || flagBg.warn;
        doc.setFillColor(...bg);
        doc.roundedRect(margin, y - 4, contentW, blockH, 2, 2, "F");

        const dot = flagDot[f.type] || flagDot.warn;
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(26, 26, 24);
        titleLines.forEach(l => { doc.text(l, margin + 12, y + 2); y += 5.5; });

        doc.setFont("helvetica", "normal");
        doc.setFontSize(8.5);
        doc.setTextColor(107, 107, 101);
        detailLines.forEach(l => { doc.text(l, margin + 12, y + 2); y += 5; });
        y += 10;
      });
      ...
      doc.text("Generated by LeaseWise · leasewise.rentals", margin, y);
      doc.text("For informational purposes only. Not legal advice.", pageW - margin, y, { align: "right" });

      doc.save("LeaseWise-Report.pdf");
    };
                  ...
                  <div style={{ position: "relative", marginTop: 4 }}>
                    ...
                    <div style={{ background: COLORS.white, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "1.25rem", textAlign: "center", marginTop: "0.5rem" }}>
                      ...
                      <div style={{ fontSize: "0.72rem", color: COLORS.inkMuted }}>
                        One-time payment · Includes PDF download · Secure checkout via Stripe
                      </div>
                    </div>
                  </div>
                )}
